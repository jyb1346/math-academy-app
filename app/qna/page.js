    ...
    <div className="min-h-screen bg-slate-100/70 pb-20 font-sans text-slate-800">
      <header className="bg-white/80 backdrop-blur-md border-b border-slate-200/80 sticky top-0 z-30 px-6 py-4 flex justify-between items-center shadow-xs">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-amber-500 to-orange-600 text-white flex items-center justify-center font-black text-lg shadow-md shadow-amber-500/20 cursor-pointer" onClick={() => router.push('/')}>
            품
          </div>
          <div>
      ...
      <main className="max-w-4xl mx-auto px-4 mt-8 space-y-4">
        <div className="bg-white p-12 rounded-3xl text-center border border-slate-200/80 space-y-3">
          ...
          <h2 className="text-base font-extrabold text-slate-800">질의응답 게시글이 없습니다.</h2>
          <p className="text-xs text-slate-400">모르는 수학 문제 질문을 남기면 선생님이 답변해 드립니다.</p>
        </div>
      </main>
    </div>
  );
