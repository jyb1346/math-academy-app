...
// 💨 선착순 마감 푸시 알림 발송 (기존 상단바 알림을 '마감'으로 자동 교체)
async function notifyFinishPush(post, eventId, targetCount) {
  try {
    let targetUserIds = [];
    ...
    if (typeof fetch !== 'undefined') {
      fetch('/api/push/send', {
        ...
        body: JSON.stringify({
          ...
          title: '💨 [황금 벌레 마감] 이번 벌레 포획 완료! ⚡',
          message: `선착순 ${targetCount}명이 황금 벌레를 모두 잡았습니다! 다음 돌발 이벤트를 기대하세요!`,
...
export async function catchLuckyBug(eventId, studentId, studentName) {
  try {
    ...
    if (existingCatches && existingCatches.length >= targetCount) {
      ...
      notifyFinishPush(post, eventId, targetCount);
  ...
  try {
    const payload = {
      ...
      content: JSON.stringify({
        ...
        escapeGimmick: Boolean(escapeGimmick),
      }),
      category: 'NOTICE',
      author_id: teacherId,
