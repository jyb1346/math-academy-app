...
export async function getJangStudentUserIds(teacherId = null) {
  try {
    let targetTeacherId = teacherId;
    if (!targetTeacherId) {
      const { data: jangTeacher } = await supabase
        .from('users')
        .select('id')
...
async function notifyFinishPush(post, eventId, summaryText) {
  try {
    ...
    if (post && post.class_id) {
      ...
      targetUserIds = await getJangStudentUserIds(post.author_id);
    }
...
export async function getActiveLuckyEvent(studentClassIds = [], studentId = null) {
  try {
      ...
      try {
        ...
        if (meta.isLuckyEvent && meta.status === 'ACTIVE') {
          ...
          if (post.class_id) {
            ...
            const jangStudentIds = await getJangStudentUserIds(post.author_id);
...
export async function hitBossRaid(eventId, studentId, studentName, damage = null) {
  try {
    const { data: post, error: postErr } = await supabase
      .from('posts')
      .select('*')
...
export async function deleteLuckyEvent(eventId) {
  try {
    await supabase.from('post_confirmations').delete().eq('post_id', eventId);
...
export async function synthesizeBugs(studentId, tier) {
  try {
    ...
    const { data: myCatches, error: fetchErr } = await supabase
      .from('post_confirmations')
    ...
    const { data: synthPost } = await supabase
      .from('posts')
      .insert([
        {
          title: synthTitle,
    ...
    if (synthPost) {
      await supabase.from('post_confirmations').insert([
        {
