...
export async function computeAvailableTargets(db, student) {
  const { data: uData } = await db.from('users').select('id, name, role, teacher_id');
  const tList = (uData || []).filter((u) => u.role === 'TEACHER' || u.role === 'HEAD_TEACHER');
  const teacherNameMap = {};
  tList.forEach((t) => { teacherNameMap[t.id] = t.name; });

  const { data: csData } = await db.from('class_students').select('class_id').eq('student_id', student.id);
  const { data: clsData } = await db.from('classes').select('id, name, teacher_id');
  const classMap = {};
