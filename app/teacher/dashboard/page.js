      ...
      <header className="bg-slate-900/80 backdrop-blur-md border-b border-slate-800 sticky top-0 z-30 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-xs bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-bold px-3.5 py-2 rounded-xl transition shadow-sm flex items-center gap-1.5"
          >
            <span>📢 게시판</span>
          </button>
          <button
      ...
      <main className="max-w-6xl mx-auto px-4 mt-8 space-y-8">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div
            ...
            className="group relative bg-gradient-to-br from-blue-600 via-indigo-600 to-indigo-800 p-6 rounded-3xl shadow-xl shadow-indigo-950/30 cursor-pointer overflow-hidden transition-all duration-300 hover:scale-[1.01] hover:shadow-2xl"
          >
            <div className="absolute top-0 right-0 -mt-8 -mr-8 w-36 h-36 bg-white/10 rounded-full blur-2xl pointer-events-none"></div>
            
            <div className="flex justify-between items-start">
              <span className="bg-white/20 backdrop-blur-md border border-white/20 text-white text-[11px] px-3 py-1 rounded-full font-bold">
                ✍️ Daily Feedback
              </span>
              <span className="text-2xl text-white/60 group-hover:text-white group-hover:translate-x-1 transition-all">→</span>
            </div>

            <div className="mt-6 space-y-1">
              <h2 className="text-xl font-black text-white">일일 학습 피드백 작성</h2>
              <p className="text-xs text-blue-100/80 font-normal">
                학생별 6대 성취도 영역 및 출결/지각 상태를 기록합니다.
              </p>
            </div>
          </div>
          ...
          <div
            ...
            className="group bg-slate-800/90 border border-slate-700/80 hover:border-slate-600 p-6 rounded-3xl shadow-lg cursor-pointer transition-all duration-300 hover:scale-[1.01]"
          >
            <div className="flex justify-between items-start">
              <span className="bg-slate-700/80 text-indigo-300 border border-slate-600 text-[11px] px-3 py-1 rounded-full font-bold">
                📋 Feedback Report
              </span>
            ...
            <div className="mt-6 space-y-1">
              <h2 className="text-xl font-black text-white">피드백 리포트 내역 조회</h2>
              <p className="text-xs text-slate-400 font-normal">
                작성된 피드백 통계와 학부모님의 수신 답장을 확인합니다.
              </p>
            </div>
          </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* 개별 등록 */}
          <div className="md:col-span-2 bg-slate-800/60 border border-slate-700/70 p-6 rounded-3xl space-y-5 shadow-lg backdrop-blur-xs">
            <div className="flex items-center gap-2 border-b border-slate-700/60 pb-4">
              ...
              <div>
                <h2 className="text-base font-extrabold text-white">개별 학생 신규 등록</h2>
                <p className="text-xs text-slate-400">학생 정보와 카카오 알림톡 수신용 학부모 연락처를 등록합니다.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleCreateSingleStudent} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 mb-1.5">학생 이름</label>
                  <input
                    type="text"
                    placeholder="홍길동"
                    value={studentName}
                    onChange={(e) => setStudentName(e.target.value)}
                    className="w-full p-3 bg-slate-900/90 border border-slate-700/80 rounded-2xl text-xs font-semibold text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition"
                  />
                </div>

                <div>
                  <label className="block text-[11px] font-bold text-slate-400 mb-1.5">아이디 / 이메일</label>
                  <input
                    type="text"
                    placeholder="hong123"
                    value={studentEmail}
                ...
                <div>
                  <label className="block text-[11px] font-bold text-amber-400 mb-1.5">📱 학부모 연락처 (알림톡)</label>
                  <input
                    type="tel"
                    placeholder="010-1234-5678"
                    value={parentPhone}
                    onChange={(e) => setParentPhone(e.target.value)}
                    className="w-full p-3 bg-amber-950/20 border border-amber-500/30 rounded-2xl text-xs font-semibold text-amber-200 placeholder-slate-600 focus:outline-none focus:border-amber-400 transition"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-2xl text-xs transition shadow-md shadow-blue-600/20"
              >
                + 학생 등록하기
              </button>
            </form>
          ...
          {/* 일괄 등록 */}
          <div className="bg-slate-800/60 border border-slate-700/70 p-6 rounded-3xl space-y-5 shadow-lg backdrop-blur-xs">
            <div className="flex items-center gap-2 border-b border-slate-700/60 pb-4">
              <span className="p-2 bg-amber-500/10 text-amber-400 rounded-xl text-base font-bold">⚡</span>
              <div>
                <h2 className="text-base font-extrabold text-white">학생 일괄 계정 생성</h2>
                <p className="text-xs text-slate-400">이름 목록을 입력해 일괄 생성합니다.</p>
              </div>
            </div>

            <form onSubmit={handleBatchCreateStudents} className="space-y-3">
              <textarea
                placeholder="학생 이름 목록 (줄바꿈 구분)&#10;김철수&#10;이영희&#10;박민수"
                value={batchNames}
                onChange={(e) => setBatchNames(e.target.value)}
                className="w-full p-3 bg-slate-900/90 border border-slate-700/80 rounded-2xl text-xs h-28 font-medium text-white placeholder-slate-500 focus:outline-none focus:border-slate-500 transition"
              />
              <button
                type="submit"
                className="w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 rounded-2xl text-xs transition shadow-sm"
              >
                일괄 계정 생성하기
              </button>
            </form>
